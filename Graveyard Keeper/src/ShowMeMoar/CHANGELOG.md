...
- Corrected the high-DPI fix instructions and dialog after player testing: the manual workaround on the mod page now says set "High DPI scaling override" to "Application" (was "System", which actually forces blurry bitmap upscaling — the opposite of what we want), and the in-game success dialog now tells you that if the main menu still looks blurry after the restart, opening the in-game Settings → Display and changing the resolution once will snap it to the correct DPI. The old "try the Zoom slider" troubleshooting tip has been removed — it was masking the blurry menu by upscaling from FHD rather than rendering at native resolution.
...
- Option to toggle color correction
- Main menu artwork fixed. Looks fine on 21:9, not so sure about 32:9+. No change at 16:9.

## 0.1.5

- Minor change related to logging messages.
