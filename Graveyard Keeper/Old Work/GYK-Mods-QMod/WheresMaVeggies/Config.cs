...
public static class Config
{
    ...
    public static Options GetOptions(bool external = false)
    {
        ...
        if (key)
        {
            ...
            if (!external)
            {
                Debug.LogWarning($"[WheresMaVeggies]: Parsed '{b}' for 'ReloadConfigKeyBind'.");
            }
        }
        else
        {
            ...
            if (!external)
            {
                Debug.LogWarning($"[WheresMaVeggies]: Failed to parse key for 'ReloadConfigKeyBind'. Setting to default F5.");
            }
        }
