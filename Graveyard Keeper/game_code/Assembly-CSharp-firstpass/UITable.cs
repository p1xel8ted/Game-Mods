...
public class UITable : UIWidgetContainer
{
  ...
  public static Vector3 RoundVector(Vector3 vec, float value = 0.5f)
  {
  ...
  public void RepositionVariableSize(List<Transform> children)
  {
    float num1 = 0.0f;
    float num2 = 0.0f;
    ...
    for (int count = children.Count; index3 < count; ++index3)
    {
      ...
      Bounds relativeWidgetBounds = NGUIMath.CalculateRelativeWidgetBounds(child, child, !this.hideInactive, this.considerChildren);
      relativeWidgetBounds.center = UITable.RoundVector(relativeWidgetBounds.center);
      relativeWidgetBounds.extents = UITable.RoundVector(relativeWidgetBounds.extents);
    ...
    for (int count = children.Count; index6 < count; ++index6)
    {
      ...
      Vector3 vec = child.localPosition with
      {
        x = num1 + bounds1.extents.x - bounds1.center.x
      };
      vec.x -= Mathf.Lerp(0.0f, bounds1.max.x - bounds1.min.x - bounds2.max.x + bounds2.min.x, pivotOffset1.x) - this.padding.x;
      if (this.direction == UITable.Direction.Down)
      {
        vec.y = -num2 - bounds1.extents.y - bounds1.center.y;
        vec.y += Mathf.Lerp(bounds1.max.y - bounds1.min.y - bounds3.max.y + bounds3.min.y, 0.0f, pivotOffset1.y) - this.padding.y;
      }
      else
      {
        vec.y = num2 + bounds1.extents.y - bounds1.center.y;
        vec.y -= Mathf.Lerp(0.0f, bounds1.max.y - bounds1.min.y - bounds3.max.y + bounds3.min.y, pivotOffset1.y) - this.padding.y;
      }
      num1 += bounds2.size.x + this.padding.x * 2f;
      vec = UITable.RoundVector(vec);
