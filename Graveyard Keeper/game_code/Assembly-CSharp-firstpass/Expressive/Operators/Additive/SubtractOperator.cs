...
// Type: Expressive.Operators.Additive.SubtractOperator
...
public class SubtractOperator : OperatorBase
{
  ...
  {
    return this.IsUnary(previousToken) ? (IExpression) new UnaryExpression(UnaryExpressionType.Minus, expressions[0] ?? expressions[1]) : (IExpression) new BinaryExpression(BinaryExpressionType.Subtract, expressions[0], expressions[1]);
  }
