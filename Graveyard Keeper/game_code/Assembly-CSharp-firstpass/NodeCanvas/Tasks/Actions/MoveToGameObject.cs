...
public class MoveToGameObject : ActionTask<NavMeshAgent>
{
  ...
  public override string info => "Seek " + this.target?.ToString();

  public override void OnExecute()
  {
    this.agent.speed = this.speed.value;
    if ((double) Vector3.Distance(this.agent.transform.position, this.target.value.transform.position) >= (double) this.agent.stoppingDistance + (double) this.keepDistance)
      return;
    this.EndAction(true);
  }
  ...
  {
    if (!((Object) this.target.value != (Object) null))
      return;
    Gizmos.DrawWireSphere(this.target.value.transform.position, this.keepDistance);
  }
}
