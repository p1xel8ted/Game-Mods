...
public class MecanimSetInt : ActionTask<Animator>
{
  ...
  public override string info
  {
    get
    {
      return $"Mec.SetInt {(string.IsNullOrEmpty(this.parameter.value) ? (object) this.parameterHashID.ToString() : (object) this.parameter.ToString())} to {this.setTo}";
    }
  }
  ...
  public override void OnExecute()
  {
    if (!string.IsNullOrEmpty(this.parameter.value))
      this.agent.SetInteger(this.parameter.value, this.setTo.value);
    else
      this.agent.SetInteger(this.parameterHashID.value, this.setTo.value);
    this.EndAction();
  }
}
