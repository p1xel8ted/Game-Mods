...
public abstract class RewiredPointerInputModule : BaseInputModule
{
  ...
  public virtual void ProcessMove(PlayerPointerEventData pointerEvent)
  {
    ...
    {
      gameObject = this.GetMouseInputSource(pointerEvent.playerId, pointerEvent.inputSourceIndex).locked ? (GameObject) null : pointerEvent.pointerCurrentRaycast.gameObject;
    }
    else
    {
  ...
  {
    if (!pointerEvent.IsPointerMoving() || (UnityEngine.Object) pointerEvent.pointerDrag == (UnityEngine.Object) null || pointerEvent.sourceType == PointerEventType.Mouse && this.GetMouseInputSource(pointerEvent.playerId, pointerEvent.inputSourceIndex).locked)
      return;
