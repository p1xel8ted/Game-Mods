...
public class PointNode(AstarPath astar) : GraphNode(astar)
{
  ...
  public void SetPosition(Int3 value) => this.position = value;
    ...
    for (int index = 0; index < this.connections.Length; ++index)
    {
      ...
      if (path.CanTraverse(connection))
      {
        ...
        else
        {
          ...
          if (pathNode.G + connectionCost + path.GetTraversalCost(connection) < pathNode1.G)
          {
