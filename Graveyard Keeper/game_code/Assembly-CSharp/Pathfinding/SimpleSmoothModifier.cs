...
public class SimpleSmoothModifier : MonoModifier
{
  ...
  public List<Vector3> CurvedNonuniform(List<Vector3> path)
  {
    ...
    for (int index = 0; index < path.Count - 1; ++index)
    {
      ...
      {
        float t = num2 * num1;
        vector3List.Add(SimpleSmoothModifier.GetPointOnCubic(a, b, tan1, tan2, t));
      }
      vector3_2 = vector3_4;
    }
  ...
  public List<Vector3> SmoothOffsetSimple(List<Vector3> path)
  {
    ...
    for (int p = 0; p < this.iterations; ++p)
    {
      ...
      for (int index = 0; index < num - 1; ++index)
      {
        ...
        bool flag3 = false;
        bool flag4 = false;
        if (index != 0 && !VectorMath.IsColinearXZ(a, b, list[index - 1]))
        {
          flag3 = true;
          flag1 = VectorMath.RightOrColinearXZ(a, b, list[index - 1]);
        }
        if (index < num - 1 && !VectorMath.IsColinearXZ(a, b, list[index + 2]))
        {
          flag4 = true;
          flag2 = VectorMath.RightOrColinearXZ(a, b, list[index + 2]);
        }
