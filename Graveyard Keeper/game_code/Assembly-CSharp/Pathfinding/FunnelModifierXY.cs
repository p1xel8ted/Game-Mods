...
// Type: Pathfinding.FunnelModifierXY
...
public class FunnelModifierXY : MonoModifier
{
  ...
  public override void Apply(Path p)
  {
    ...
    if (!FunnelModifierXY.RunFunnel(vector3List1, vector3List2, funnelPath))
    {
  ...
  public static bool RunFunnel(List<Vector3> left, List<Vector3> right, List<Vector3> funnelPath)
  {
    ...
    if (p == left[1])
      p = right[2];
    while (VectorMathXY.IsColinearXY(left[0], left[1], right[1]) || VectorMathXY.RightOrColinearXY(left[1], right[1], p) == VectorMathXY.RightOrColinearXY(left[1], right[1], left[0]))
    {
      left.RemoveAt(1);
      right.RemoveAt(1);
      if (left.Count <= 3)
    ...
    for (int index = 2; index < left.Count; ++index)
    {
      ...
      if ((double) VectorMathXY.SignedTriangleAreaTimes2XY(a, b2, c2) >= 0.0)
      {
        if (a == b2 || (double) VectorMathXY.SignedTriangleAreaTimes2XY(a, b1, c2) <= 0.0)
        {
          b2 = c2;
          num1 = index;
        }
      ...
      if ((double) VectorMathXY.SignedTriangleAreaTimes2XY(a, b1, c1) <= 0.0)
      {
        if (a == b1 || (double) VectorMathXY.SignedTriangleAreaTimes2XY(a, b2, c1) >= 0.0)
        {
          b1 = c1;
          num2 = index;
        }
