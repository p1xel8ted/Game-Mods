...
public class Plugin : BaseUnityPlugin
{
    ...
    internal const string PluginName = "TrackIt!";
    ...
    internal const string WishIconSpriteString = "WishIcon";
    ...
    public void Awake()
    {
        ...
        ColouriseQuestTracker.SettingChanged += (_, _) =>
        {
        ...
        QuestTrackerHeaderColour.SettingChanged += (_, _) =>
        {
            QuestTrackerNeedsUpdating = true;
        };
        ...
        QuestTrackerCompletedColour.SettingChanged += (_, _) =>
        {
            QuestTrackerNeedsUpdating = true;
        };
        ...
        QuestTrackerFailedColour.SettingChanged += (_, _) =>
        {
            QuestTrackerNeedsUpdating = true;
        };
        ...
        QuestTrackerHalfwayColour.SettingChanged += (_, _) =>
        {
            QuestTrackerNeedsUpdating = true;
        };
        ...
        MaxQuestCount.SettingChanged += (_, _) =>
        {
            QuestTrackerNeedsUpdating = true;
        };
        ...
        {
            //QuestTrackerNeedsUpdating = true;
            QuestTrackerNeedsRebuilding = true;
        };
        ...
        UseGameFont.SettingChanged += (_, _) =>
        {
            //QuestTrackerNeedsUpdating = true;
            QuestTrackerNeedsRebuilding = true;
        };
        ...
        QuestLineGap.SettingChanged += (_, _) =>
        {
            //QuestTrackerNeedsUpdating = true;
            QuestTrackerNeedsRebuilding = true;
        };
        ...
        ShowQuestTargetImages.SettingChanged += (_, _) =>
        {
            QuestTracker.UpdateQuestTargetImageVisibility();
        };
    ...
    private static List<ActiveQuest> _currentQuests = [];
    internal static bool QuestTrackerNeedsUpdating { get; set; }
    internal static bool QuestTrackerNeedsRebuilding { get; set; }
    private static IEnumerator CheckQuests()
    {
        while (true)
        {
            var ok = HeroMerchant.Instance is not null && GameManager.Instance is not null;
            if (ok)
            {
    ...
    private void Start()
    {
        ...
        Plugin.LOG.LogWarning("Quest Tracker started!");
    }

    private void Update()
    {
        if ((InputManager.ActiveDevice.RightStickButton.WasPressed || InputManager.ActiveDevice.LeftStickButton.WasPressed || QuestTrackerToggleKeybind.Value.IsUp()) && QuestTracker.QuestTrackerObject is not null)
        {
    ...
    private static bool AreActiveQuestsEqual(ActiveQuest quest1, ActiveQuest quest2)
    {
        ...
        return quest1.quest == quest2.quest && quest1.completed == quest2.completed && quest1.failed == quest2.failed && quest1.completeDay == quest2.completeDay && quest1.giverVisitorPrefabName == quest2.giverVisitorPrefabName;
    }
