...
public class BossPortalEffect : MonoBehaviour
{
  ...
  public void Start()
  {
    ...
    foreach (ParticleSystem particleSystem in componentsInChildren)
      particleSystem.Play();
    this.PortalContainer.transform.localScale = Vector3.zero;
    this.PortalContainer.SetActive(false);
    this.PortalInteraction.enabled = false;
    DG.Tweening.Sequence sequence = DOTween.Sequence();
    sequence.AppendInterval(2f);
    sequence.AppendCallback((TweenCallback) (() =>
    {
      this.PortalInteraction.enabled = true;
      this.PortalContainer.SetActive(true);
    }));
    sequence.Append((Tween) this.PortalContainer.transform.DOScale(Vector3.one, 1f).SetEase<TweenerCore<Vector3, Vector3, VectorOptions>>(Ease.OutBack));
