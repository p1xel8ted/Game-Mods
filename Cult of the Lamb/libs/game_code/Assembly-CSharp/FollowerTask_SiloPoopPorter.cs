...
// Type: FollowerTask_SiloPoopPorter
...
#nullable disable
public class FollowerTask_SiloPoopPorter(
  ...
  int targetStructure) : FollowerTask_LogisticsPorter(logisticsStructure, rootStructure, targetStructure)
{
  public override List<InventoryItem> CollectItemsFromStructure(StructureBrain structureBrain)
  {
    ...
    {
      if (InventoryItem.AllPoops.Contains((InventoryItem.ITEM_TYPE) structureBrain.Data.Inventory[index].type))
    ...
    foreach (Interaction_CompostBinDeadBody compostBinDeadBody in Interaction_CompostBinDeadBody.DeadBodyCompost)
      compostBinDeadBody.UpdateImages();
