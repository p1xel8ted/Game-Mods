...
public class Interaction_SiloSeeder : Interaction
{
  ...
  {
    this.Interactable = (double) this.GetCompostAndAirCount() < (double) this.StructureBrain.Capacity;
    int num = Mathf.Min(this.GetCompostAndAirCount(), (int) this.StructureBrain.Capacity);
    if (LocalizeIntegration.IsArabic())
      this.Label = (double) this.GetCompostAndAirCount() >= (double) this.StructureBrain.Capacity ? $"{ScriptLocalization.Interactions.Full} ){LocalizeIntegration.FormatCurrentMax(num.ToString(), this.StructureBrain.Capacity.ToString())}(" : ScriptLocalization.Interactions_Bank.Deposit;
    else
      this.Label = (double) this.GetCompostAndAirCount() >= (double) this.StructureBrain.Capacity ? $"{ScriptLocalization.Interactions.Full} ({LocalizeIntegration.FormatCurrentMax(num.ToString(), this.StructureBrain.Capacity.ToString())})" : ScriptLocalization.Interactions_Bank.Deposit;
  }
