...
public class UIPlayerUpgradesMenuController : UIMenuBase
{
  ...
  public void Start()
  {
    ...
    bool flag = !DataManager.Instance.SurvivalModeActive;
    if (Inventory.GetItemQuantity(InventoryItem.ITEM_TYPE.MONSTER_HEART) >= 1 && DataManager.Instance.BeatenYngya && !UpgradeSystem.GetUnlocked(UpgradeSystem.Type.Ability_WinterChoice) || UpgradeSystem.GetUnlocked(UpgradeSystem.Type.Ability_WinterChoice))
      flag = true;
    this._crownHeader.gameObject.SetActive(flag);
