...
public class UpgradeTreeComputeShaderController : MonoBehaviour
{
  ...
  [SerializeField]
  public List<UpgradeTreeNode> _treeNodes;
  [SerializeField]
  ...
  public void Update()
  {
    ...
    for (int index = 0; index < this._treeNodes.Count; ++index)
    {
      ...
      this._data[index].Influence = this._treeNodes[index].UnlockedWeight + this._treeNodes[index].HiddenWeight;
    }
    ...
    this._offset.x /= this._scrollRect.viewport.rect.width;
    this._offset.y /= this._scrollRect.viewport.rect.height;
