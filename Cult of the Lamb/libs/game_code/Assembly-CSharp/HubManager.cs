...
public class HubManager : BaseMonoBehaviour
{
  ...
  public EventInstance hubAtmosInstance;
  public Door North;
  public Door East;
  public Door South;
  public Door West;
  ...
  public IEnumerator PlaceAndPositionPlayer()
  {
    ...
    if (hubManager.DoFirstArrivalRoutine)
    {
      ...
      if (PlayerFarming.LastLocation == FollowerLocation.Graveyard_Location && PlayerFarming.Location == FollowerLocation.LambTown)
        entranceDoor = Door.GetEntranceDoor(Door.Direction.North);
  ...
  public IEnumerator DelayPlayerGoToAndStop()
  {
    yield return (object) new WaitForSeconds(0.5f);
    Door door = Door.GetEntranceDoor();
    if (PlayerFarming.LastLocation == FollowerLocation.Graveyard_Location && PlayerFarming.Location == FollowerLocation.LambTown)
      door = Door.GetEntranceDoor(Door.Direction.North);
    float entranceGoToDistance = door.EntranceGoToDistance;
    foreach (PlayerFarming player in PlayerFarming.players)
      player.GoToAndStop(player.transform.position + door.GetDoorDirection() * entranceGoToDistance, IdleOnEnd: true, GoToCallback: (System.Action) (() =>
      {
