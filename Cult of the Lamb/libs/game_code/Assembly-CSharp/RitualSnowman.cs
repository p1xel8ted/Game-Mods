...
public class RitualSnowman : Ritual
{
  ...
  public IEnumerator RitualRoutine()
  {
    ...
    yield return (object) ritualSnowman.StartCoroutine((IEnumerator) ritualSnowman.WaitFollowersFormCircle());
    ...
    for (int index = 0; index < 2 && followerBrainList.Count != 0; ++index)
    {
      ...
      if ((bool) (UnityEngine.Object) followerById)
        ritualSnowman.StartCoroutine((IEnumerator) ritualSnowman.MoveFollower(followerById, index));
    }
    ...
    foreach (FollowerBrain brain in Ritual.FollowerToAttendSermon)
    {
      if (brain != null)
      {
        ...
        num1 += Delay;
        ritualSnowman.StartCoroutine((IEnumerator) ritualSnowman.DelayFollowerReaction(brain, Delay, !brain._directInfoAccess.IsSnowman));
  ...
  public IEnumerator MoveFollower(Follower follower, int index)
  {
    ...
    while (waiting)
      yield return (object) null;
    yield return (object) ritualSnowman.StartCoroutine((IEnumerator) follower.GoToRoutine(PlayerFarming.Instance.transform.position + positions[index]));
