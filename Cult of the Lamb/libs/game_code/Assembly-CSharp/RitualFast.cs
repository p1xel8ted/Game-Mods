...
public class RitualFast : Ritual
{
  ...
  public IEnumerator RitualRoutine()
  {
    ...
    yield return (object) ritualFast.StartCoroutine(ritualFast.WaitFollowersFormCircle());
    ...
    for (int index = 0; index < 4 && followerBrainList.Count != 0; ++index)
    {
      ...
      if ((bool) (UnityEngine.Object) followerById)
        ritualFast.StartCoroutine(ritualFast.MoveFollower(followerById, index));
    }
    ...
    foreach (FollowerBrain brain in Ritual.FollowerToAttendSermon)
    {
      ...
      num += Delay;
      ritualFast.StartCoroutine(ritualFast.DelayFollowerReaction(brain, Delay));
    }
  ...
  public IEnumerator MoveFollower(Follower follower, int index)
  {
    ...
    yield return (object) ritualFast.StartCoroutine(follower.GoToRoutine(ChurchFollowerManager.Instance.RitualCenterPosition.position + positions[index]));
