...
#nullable disable
public class EnemyTriDash : UnitObject
{
  [SerializeField]
  ...
  {
    EnemyTriDash enemyTriDash = this;
    yield return (object) new WaitForEndOfFrame();
    while (true)
    {
      while (GameManager.RoomActive)
      {
        if (enemyTriDash.state.CURRENT_STATE == StateMachine.State.Idle && (double) (enemyTriDash.IdleWait -= Time.deltaTime) <= 0.0 && (double) GameManager.GetInstance().CurrentTime > (double) enemyTriDash.initialMovementDelay)
        {
          if (enemyTriDash.wander)
            enemyTriDash.GetNewTargetPosition();
          else
            enemyTriDash.Flee();
          enemyTriDash.speed = enemyTriDash.maxSpeed * enemyTriDash.SpeedMultiplier;
        }
        if ((UnityEngine.Object) enemyTriDash.TargetEnemy != (UnityEngine.Object) null && !enemyTriDash.Attacking && !enemyTriDash.IsStunned)
          enemyTriDash.state.LookAngle = Utils.GetAngle(enemyTriDash.transform.position, enemyTriDash.TargetEnemy.transform.position);
        else
          enemyTriDash.state.LookAngle = enemyTriDash.state.facingAngle;
        if (enemyTriDash.MovingAnimation != "" && (double) GameManager.GetInstance().CurrentTime > (double) enemyTriDash.initialMovementDelay)
        {
          if (enemyTriDash.state.CURRENT_STATE == StateMachine.State.Moving && enemyTriDash.Spine.AnimationName != enemyTriDash.MovingAnimation)
            enemyTriDash.SetAnimation(enemyTriDash.MovingAnimation, true);
          if (enemyTriDash.state.CURRENT_STATE == StateMachine.State.Idle && enemyTriDash.Spine.AnimationName != enemyTriDash.IdleAnimation)
  ...
  {
    EnemyTriDash enemyTriDash = this;
    enemyTriDash.Attacking = true;
    enemyTriDash.ClearPaths();
    if (!((UnityEngine.Object) enemyTriDash.TargetEnemy == (UnityEngine.Object) null))
    {
      int CurrentAttack = 0;
      float time = 0.0f;
      while (++CurrentAttack <= enemyTriDash.NumberOfAttacks)
      {
        enemyTriDash.SetAnimation(enemyTriDash.SignPostAttackAnimation, enemyTriDash.LoopSignPostAttackAnimation);
        enemyTriDash.state.CURRENT_STATE = StateMachine.State.SignPostAttack;
        AudioManager.Instance.PlayOneShot(enemyTriDash.warningSfx, enemyTriDash.transform.position);
        enemyTriDash.TargetEnemy = enemyTriDash.GetClosestTarget();
        enemyTriDash.LookAtTarget();
        float Progress = 0.0f;
        float Duration = enemyTriDash.SignPostAttackDuration;
        while ((double) (Progress += Time.deltaTime) < (double) Duration / (double) enemyTriDash.Spine.timeScale)
        {
          enemyTriDash.SimpleSpineFlash.FlashWhite(Progress / Duration);
          float num = Mathf.Round(Utils.GetAngle(enemyTriDash.transform.position, enemyTriDash.TargetEnemy.transform.position) / 45f) * 45f;
          if ((UnityEngine.Object) enemyTriDash.Aiming != (UnityEngine.Object) null)
          {
            if (!enemyTriDash.Aiming.gameObject.activeSelf && (double) enemyTriDash.Spine.timeScale != 9.9999997473787516E-05)
              enemyTriDash.Aiming.transform.eulerAngles = new Vector3(0.0f, 0.0f, num);
            enemyTriDash.Aiming.gameObject.SetActive(true);
            float z = Mathf.LerpAngle(enemyTriDash.Aiming.transform.eulerAngles.z, num, 5f * Time.deltaTime);
            if ((double) enemyTriDash.Spine.timeScale != 9.9999997473787516E-05)
              enemyTriDash.Aiming.transform.eulerAngles = new Vector3(0.0f, 0.0f, z);
            if ((double) enemyTriDash.flashTickTimer >= 0.11999999731779099 && BiomeConstants.Instance.IsFlashLightsActive)
            {
              enemyTriDash.Aiming.color = enemyTriDash.Aiming.color == Color.red ? Color.white : Color.red;
              enemyTriDash.flashTickTimer = 0.0f;
            }
            enemyTriDash.flashTickTimer += Time.deltaTime;
          }
          yield return (object) null;
        }
        enemyTriDash.Aiming.gameObject.SetActive(false);
        enemyTriDash.SimpleSpineFlash.FlashWhite(false);
        enemyTriDash.DisableForces = true;
        enemyTriDash.Force = (Vector3) (new Vector2(2500f * Mathf.Cos(enemyTriDash.state.LookAngle * ((float) Math.PI / 180f)), 2500f * Mathf.Sin(enemyTriDash.state.LookAngle * ((float) Math.PI / 180f))) * enemyTriDash.AttackForceModifier);
        enemyTriDash.rb.AddForce((Vector2) enemyTriDash.Force);
        enemyTriDash.damageColliderEvents.SetActive(true);
        AudioManager.Instance.PlayOneShot(enemyTriDash.attackSfx, enemyTriDash.transform.position);
        enemyTriDash.state.CURRENT_STATE = StateMachine.State.RecoverFromAttack;
        enemyTriDash.SetAnimation(enemyTriDash.AttackAnimation);
        enemyTriDash.AddAnimation(enemyTriDash.IdleAnimation, true);
        time = 0.0f;
        while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < (double) enemyTriDash.AttackDuration)
          yield return (object) null;
        enemyTriDash.damageColliderEvents.SetActive(false);
        time = 0.0f;
        while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < 0.5)
          yield return (object) null;
      }
      time = 0.0f;
      while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < (double) enemyTriDash.AttackDuration * 0.5)
        yield return (object) null;
      enemyTriDash.DisableForces = false;
      enemyTriDash.state.CURRENT_STATE = StateMachine.State.Idle;
      enemyTriDash.IdleWait = 0.0f;
      enemyTriDash.AttackDelay = enemyTriDash.AttackDelayTime;
      enemyTriDash.Attacking = false;
      enemyTriDash.TargetEnemy = (Health) null;
      enemyTriDash.GetNewTargetPosition();
    }
  }
  ...
  public IEnumerator ApplyForceRoutine(GameObject Attacker)
  {
    EnemyTriDash enemyTriDash = this;
    enemyTriDash.DisableForces = true;
    enemyTriDash.Angle = Utils.GetAngle(Attacker.transform.position, enemyTriDash.transform.position) * ((float) Math.PI / 180f);
    enemyTriDash.Force = (Vector3) (new Vector2(1500f * Mathf.Cos(enemyTriDash.Angle), 1500f * Mathf.Sin(enemyTriDash.Angle)) * enemyTriDash.KnockbackModifier);
    enemyTriDash.rb.AddForce((Vector2) (enemyTriDash.Force * enemyTriDash.KnockBackModifier));
    float time = 0.0f;
    while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < 0.5)
      yield return (object) null;
    enemyTriDash.DisableForces = false;
  ...
  public IEnumerator ApplyForceRoutine(Vector3 forcePosition)
  {
    EnemyTriDash enemyTriDash = this;
    enemyTriDash.DisableForces = true;
    enemyTriDash.Angle = Utils.GetAngle(forcePosition, enemyTriDash.transform.position) * ((float) Math.PI / 180f);
    enemyTriDash.Force = (Vector3) (new Vector2(1500f * Mathf.Cos(enemyTriDash.Angle), 1500f * Mathf.Sin(enemyTriDash.Angle)) * enemyTriDash.KnockbackModifier);
    enemyTriDash.rb.AddForce((Vector2) enemyTriDash.Force);
    float time = 0.0f;
    while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < 0.5)
  ...
  public IEnumerator HurtRoutine()
  {
    ...
    enemyTriDash.Attacking = false;
    enemyTriDash.ClearPaths();
    enemyTriDash.state.CURRENT_STATE = StateMachine.State.KnockBack;
    enemyTriDash.SetAnimation(enemyTriDash.IdleAnimation, true);
    float time = 0.0f;
    while ((double) (time += Time.deltaTime * enemyTriDash.Spine.timeScale) < 0.5)
      yield return (object) null;
