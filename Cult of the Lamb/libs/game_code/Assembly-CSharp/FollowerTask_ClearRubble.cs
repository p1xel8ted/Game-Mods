...
public class FollowerTask_ClearRubble : FollowerTask_AssistPlayerBase
{
  ...
  public new bool _helpingPlayer;
  public int _rubbleID;
  public Structures_Rubble rubble;
  public float _gameTimeSinceLastProgress;
  public List<Structures_Rubble> cachedRubbles = new List<Structures_Rubble>();
  ...
  public Structures_Rubble GetNextRubble()
  {
    ...
    StructureManager.TryGetAllAvailableRubble(ref this.cachedRubbles, this.Location);
    foreach (Structures_Rubble cachedRubble in this.cachedRubbles)
    {
      if ((UnityEngine.Object) followerById == (UnityEngine.Object) null)
      {
        nextRubble = cachedRubble;
        break;
      }
      float num3 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, cachedRubble.Data.Position);
      if (cachedRubble.RockSize == 0 && (double) num3 < (double) num2)
      {
        float num4 = num3 + (cachedRubble.Data.Prioritised ? 0.0f : 1000f);
    ...
    if (nextRubble == null)
    {
      foreach (Structures_Rubble cachedRubble in this.cachedRubbles)
      {
        ...
        float num5 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, cachedRubble.Data.Position);
        if (cachedRubble.RockSize == 1 && (double) num5 < (double) num2)
        {
          float num6 = num5 + (cachedRubble.Data.Prioritised ? 0.0f : 1000f);
  ...
  public override void TaskTick(float deltaGameTime)
  {
    ...
    if (PlayerFarming.Location != FollowerLocation.Base || this.State != FollowerTaskState.Doing || (double) Vector3.Distance(this.Brain.LastPosition, this.rubble.Data.Position) <= 4.0)
      return;
  ...
  public void OnPlayerActivatingEnd(Rubble rubble)
  {
