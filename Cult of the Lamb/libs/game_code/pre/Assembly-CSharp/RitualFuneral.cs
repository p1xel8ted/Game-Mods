...
public class RitualFuneral : Ritual
{
  ...
  protected override UpgradeSystem.Type RitualType => UpgradeSystem.Type.Ritual_Funeral;
  ...
  private IEnumerator RitualRoutine()
  {
    ...
    List<FollowerInfo> followerInfo1 = new List<FollowerInfo>((IEnumerable<FollowerInfo>) DataManager.Instance.Followers_Dead);
    for (int index = followerInfo1.Count - 1; index >= 0; --index)
    {
      if (followerInfo1[index].HadFuneral)
      {
        followerInfo1.RemoveAt(index);
      }
      else
      {
        bool flag = false;
        ...
        {
          if (structureBrain.Data.FollowerID == followerInfo1[index].ID)
          {
            flag = true;
            break;
          }
    ...
    followerSelectInstance.Show(followerInfo1, followerSelectionType: ritualFuneral.RitualType);
  ...
  private IEnumerator ContinueRitual(FollowerInfo deadFollowerInfo, Structures_Grave grave)
  {
    RitualFuneral ritualFuneral = this;
    AudioManager.Instance.PlayOneShot("event:/ritual_sacrifice/select_follower", PlayerFarming.Instance.gameObject);
    ...
    GameManager.GetInstance().CamFollowTarget.ForceSnapTo(grave.Data.Position - GameManager.GetInstance().CamFollowTarget.transform.forward * 3f);
