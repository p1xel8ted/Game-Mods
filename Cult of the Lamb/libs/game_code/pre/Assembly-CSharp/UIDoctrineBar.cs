...
public class UIDoctrineBar : BaseMonoBehaviour
{
  ...
  public IEnumerator Show(float xp, SermonCategory CurrentSermonCategory)
  {
    ...
    float Progress = 0.0f;
    float Duration = 0.5f;
    this.SermonXPText.text = $"{(object) Mathf.RoundToInt(xp * 10f)}/{(object) Mathf.RoundToInt(this.DoctrineXPTarget * 10f)}";
  ...
  public IEnumerator UpdateFirstBar(float xp, float duration)
  {
    ...
    while ((double) (Progress += Time.deltaTime) < (double) duration)
    {
      ...
      uiDoctrineBar.SermonXPText.text = $"{(object) Mathf.RoundToInt(xp * 10f)}/{(object) Mathf.RoundToInt(uiDoctrineBar.DoctrineXPTarget * 10f)}";
      yield return (object) null;
    }
