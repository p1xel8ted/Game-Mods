...
public class WorldGen : BaseMonoBehaviour
{
  ...
  private void OnDestroy() => WorldGen.Instance = (WorldGen) null;
  ...
  private void CreateTutorialRooms()
  {
    ...
    while (++num1 < 3)
    {
      ...
      gameObject.name = $"Room: {(object) num1}_{(object) num2}";
    ...
    for (int index1 = 0; index1 < this.width; ++index1)
    {
      for (int index2 = 0; index2 < this.height; ++index2)
      {
        ...
        else
        {
          ...
          gameObject.transform.parent = this.gameObject.transform;
          gameObject.name = $"Room: {(object) index1}_{(object) index2}";
        }
        room.x = index1;
        room.y = index2;
        WorldGen.rooms.Add(room);
  ...
  private void addLink(WorldGen.dir _dir, Room r)
  {
    if (!this.InstantiatePrefabs)
      return;
