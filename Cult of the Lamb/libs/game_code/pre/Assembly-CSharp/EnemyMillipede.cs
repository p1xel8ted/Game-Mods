...
public class EnemyMillipede : UnitObject
{
  ...
  protected virtual void Start() => this.LookAtAngle((float) Random.Range(0, 360));
  ...
  public override void Update()
  {
    if (!this.DisableForces)
    {
      ...
      if ((double) Time.time > (double) this.directionChanger)
      {
      ...
      if ((Object) (raycastHit2D1 = Physics2D.Raycast(position1, normalized1, (float) rayDistance, avoidLayers1)).collider != (Object) null)
      {
        ...
        float degree1 = Mathf.Repeat(angle + 90f, 360f);
        ...
        float TargetAngle = Mathf.Repeat(this.state.facingAngle + Random.Range(this.turnExaggerationRange.x, this.turnExaggerationRange.y), 360f);
  ...
  public void DamageFromBody(GameObject attacker, Vector3 attackLocation, float damage)
  {
    if (this.damaged)
      return;
    this.health.DealDamage(damage, attacker, attackLocation);
