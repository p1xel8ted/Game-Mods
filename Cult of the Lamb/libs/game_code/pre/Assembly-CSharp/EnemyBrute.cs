...
public class EnemyBrute : UnitObject
{
  ...
  private EnemyBrute.AttackTypes AttackType;
  ...
  private void Start() => this.state.facingAngle = (float) UnityEngine.Random.Range(0, 360);
  ...
  private void OnSpineEvent(string EventName)
  {
    switch (EventName)
    {
      case "throw":
        ...
        UnityEngine.Object.Instantiate<GameObject>(this.RockToThrow, this.TargetObject.transform.position, Quaternion.identity, this.transform.parent).GetComponent<BruteRock>().Play(this.ThrowBone.transform.position);
        break;
      case "shake":
        if (!this.simpleSpineAnimator.IsVisible)
          break;
  ...
  private IEnumerator WaitForEnemy()
  {
    ...
    while ((UnityEngine.Object) (enemyBrute.TargetObject = GameObject.FindWithTag("Player")) == (UnityEngine.Object) null)
    ...
    while (Loop)
    {
      switch (enemyBrute1.state.CURRENT_STATE)
      {
        case StateMachine.State.Moving:
          ...
          if ((double) Vector2.Distance((Vector2) enemyBrute1.transform.position, (Vector2) enemyBrute1.TargetPosition) < 2.0)
          {
          ...
          switch (enemyBrute1.AttackType)
          {
            case EnemyBrute.AttackTypes.Area:
              if ((double) (enemyBrute1.state.Timer += Time.deltaTime) >= 1.0)
              {
                ...
                GameManager.GetInstance().HitStop();
                enemyBrute1.ParticleSystem.Play();
                enemyBrute1.TargetWarning.gameObject.SetActive(false);
                if ((UnityEngine.Object) enemyBrute1.damageColliderEvents != (UnityEngine.Object) null)
                {
                  enemyBrute1.damageColliderEvents.SetActive(true);
                  break;
                }
                break;
              }
