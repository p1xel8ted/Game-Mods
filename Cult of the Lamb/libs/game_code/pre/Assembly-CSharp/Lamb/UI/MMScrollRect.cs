...
public class MMScrollRect : ScrollRect
{
  ...
  private float MaxY
  {
    get
    {
      ...
      double a = height1 - height2;
      rect = this.viewport.rect;
      double height3 = (double) rect.height;
      return Mathf.Max((float) a, (float) height3);
    }
  }
  ...
  private IEnumerator DeferredDefaultCheck(Selectable selectable)
  {
    ...
    Vector3 position2 = mmScrollRect.viewport.transform.position;
    Rect rect2 = mmScrollRect.viewport.rect;
    ...
    if ((double) vector3_3.y > (double) vector3_1.y || (double) vector3_4.y < (double) vector3_2.y)
    {
      ...
      {
        x = 0.0f,
        y = -Mathf.Clamp(mmScrollRect.content.InverseTransformPoint(selectable.transform.position).y + mmScrollRect.viewport.rect.height * 0.5f, -mmScrollRect.MaxY, mmScrollRect.MinY)
      };
      mmScrollRect.content.anchoredPosition = vector2;
    }
  }
  ...
  {
    if (InputManager.General.MouseInputActive || !selectable.transform.IsChildOf(this.transform))
      return;
    float canvasScale = MMCanvasScaler.CanvasScale;
    Vector3 position1 = selectable.transform.position;
    Rect rect1 = selectable.GetComponent<RectTransform>().rect;
    ...
    else if ((double) MonoSingleton<UINavigatorNew>.Instance.RecentMoveVector.y < 0.0)
    {
      ...
      if ((UnityEngine.Object) selectableOnDown == (UnityEngine.Object) null || !selectableOnDown.transform.IsChildOf((Transform) this.content))
  ...
  public IEnumerator DoScrollTo(Vector2 to, Vector2 from)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
    float time = Mathf.Clamp(Mathf.Abs(to.y - from.y) / mmScrollRect.viewport.rect.height, 0.0f, 1f) * mmScrollRect._scrollRectConfiguration.MoveToTravelTime;
    // ISSUE: reference to a compiler-generated field
