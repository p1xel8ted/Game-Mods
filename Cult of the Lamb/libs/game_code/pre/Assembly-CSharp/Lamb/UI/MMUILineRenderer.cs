...
public class MMUILineRenderer : MaskableGraphic, ISerializationCallbackReceiver
{
  ...
  [NonSerialized]
  private MMUILineRenderer.Branch _root = new MMUILineRenderer.Branch();
  ...
  public MMUILineRenderer.Branch Root => this._root;
  ...
  {
    this._root = JsonUtility.FromJson<MMUILineRenderer.Branch>(this._serializedRoot);
  }
  ...
  private float GetLength(List<MMUILineRenderer.BranchPoint> branch)
  {
    float length = 0.0f;
