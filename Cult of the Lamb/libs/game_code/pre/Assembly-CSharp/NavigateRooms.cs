...
public class NavigateRooms : BaseMonoBehaviour
{
  ...
  private void Start() => NavigateRooms.instance = this;
  ...
  private void Update()
  {
    if (!this.init)
    {
      ...
      this.text.text = $"X: {(object) NavigateRooms.CurrentX}   Y: {(object) NavigateRooms.CurrentY}{(NavigateRooms.r.isHome ? (object) " HOME" : (object) "")}{(NavigateRooms.r.isEntranceHall ? (object) " ENTRANCE HALL" : (object) "")}{(NavigateRooms.r.pointOfInterest ? (object) " POINT OF INTEREST" : (object) "")}";
      this.init = true;
    }
