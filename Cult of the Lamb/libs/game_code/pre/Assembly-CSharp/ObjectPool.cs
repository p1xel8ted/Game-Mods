...
public sealed class ObjectPool : BaseMonoBehaviour
{
  ...
  private void Start()
  {
    Debug.Log((object) "OBJECT POOL: Start");
    if (this.startupPoolMode != ObjectPool.StartupPoolMode.Start)
      return;
    ObjectPool.CreateStartupPools();
  }

  private void OnDestroy() => Debug.Log((object) "OBJECT POOL: Destroy");
  ...
  public static IEnumerator PoolReset()
  {
    foreach (KeyValuePair<GameObject, List<GameObject>> pooledObject in ObjectPool.instance.pooledObjects)
    {
      ...
      Debug.Log((object) ("Clearing Pool:" + pooledObject.Key.name));
  ...
  {
    ObjectPool.CreatePool(prefab.gameObject, initialPoolSize, updateExisting);
  }
    ...
    Action<GameObject> callback)
  {
    if (string.IsNullOrEmpty(path))
      return;
    GameManager.GetInstance().StartCoroutine((IEnumerator) ObjectPool.SpawnIE(path, position, rotation, parent, callback));
  }
    ...
    Action<GameObject> callback)
  {
    ...
    else
    {
      ...
      Debug.Log((object) ("OBJECT POOL, load addressable: " + path));
  ...
  public static void DestroyAll()
  {
    ...
    Debug.Log((object) ("OBJECT POOL Release Addressables" + (object) ObjectPool.instance.loadedAddressables.Count));
