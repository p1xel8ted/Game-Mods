...
public class RoomManager : BaseMonoBehaviour
{
  ...
  private void OnWorldGenerated() => this.StartCoroutine((IEnumerator) this.DoWorldGeneration());
  ...
  private IEnumerator CreateRooms()
  {
    if (!this.TestSingleRoom)
    {
      ...
      while (++i < WorldGen.rooms.Count)
      {
        ...
        this.CurrentRoomPrefab.ID = $"Room_{(object) room.x}_{(object) room.y}";
  ...
  {
    Transform RoomToPlaceTransform = RoomInfo.GetByID($"Room_{(object) RoomToPlace.x}_{(object) RoomToPlace.y}").transform;
  ...
  private void ChangeRoom()
  {
    ...
    {
      this.CurrentRoomPrefab = RoomInfo.GetByID($"Room_{(object) RoomManager.r.x}_{(object) RoomManager.r.y}");
  ...
  private void UnlockDoors() => HUD_Timer.TimerPaused = false;
