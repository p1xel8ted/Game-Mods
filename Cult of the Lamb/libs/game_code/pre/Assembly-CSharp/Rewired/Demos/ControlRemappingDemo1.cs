...
public class ControlRemappingDemo1 : MonoBehaviour
{
  ...
  private void Unsubscribe() => this.inputMapper.RemoveAllEventListeners();
  ...
  private void Open() => this.showMenu = true;
  ...
  private void ShowDialog() => this.dialog.Update();
  ...
  private void DrawCalibrationWindow(string title, string message)
  {
    ...
    else
    {
      ...
      if (calibration.selectedElementIdentifierId >= 0)
      {
        ...
        GUILayout.Label("Calibrated Value: " + (object) calibration.joystick.GetAxisById(calibration.selectedElementIdentifierId));
        GUILayout.Label("Zero: " + (object) axis.calibratedZero);
        GUILayout.Label("Min: " + (object) axis.calibratedMin);
        GUILayout.Label("Max: " + (object) axis.calibratedMax);
        GUILayout.Label("Dead Zone: " + (object) axis.deadZone);
        GUILayout.Space(15f);
  ...
  private void PlayerSelectionChanged() => this.ClearControllerSelection();

  private void ControllerSelectionChanged() => this.ClearMapSelection();
  ...
  private class DialogHelper
  {
    ...
    private void StateChanged(float delay) => this.StartBusyTimer(delay);
    ...
    private void ResetTimers() => this._busyTimerRunning = false;
