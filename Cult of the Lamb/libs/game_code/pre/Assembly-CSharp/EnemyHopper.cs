...
public class EnemyHopper : UnitObject
{
  ...
  protected virtual void UpdateStateIdle()
  {
    ...
    if ((double) this.gm.TimeSince(this.idleTimestamp) >= (double) num)
      this.SimpleSpineFlash.FlashWhite((float) (0.75 * ((double) this.gm.TimeSince(this.idleTimestamp) - (double) num)) / this.signPostDur);
    if ((double) this.gm.TimeSince(this.idleTimestamp) < (double) this.idleDur)
      return;
  ...
  protected virtual void UpdateStateMoving()
  {
    ...
    this.canBeParried = (double) this.gm.TimeSince(this.hoppingTimestamp) <= (double) this.attackParryWindow;
  ...
  {
    this.speed = 0.0f;
    if ((double) this.gm.TimeSince(this.stunnedTimestamp) < (double) this.stunnedDur)
      return;
    this.state.CURRENT_STATE = StateMachine.State.Idle;
  }
  ...
  {
    if ((double) this.gm.TimeSince(this.chargingTimestamp) < (double) this.chargingDuration || !this.canLayEggs || EnemyHopper.EnemyHoppers.Count >= EnemyHopper.maxHoppersPerRoom || EnemyEgg.EnemyEggs.Count >= EnemyHopper.maxEggsPerRoom)
      return;
    this.LayEgg();
  ...
  {
    this.speed = 0.0f;
    if ((double) this.gm.TimeSince(this.dancingTimestamp) < (double) this.dancingDuration)
      return;
    this.state.CURRENT_STATE = StateMachine.State.Idle;
  }
  ...
  protected bool TargetIsVisible()
  {
    ...
    return (double) a <= (double) this.VisionRange && this.CheckLineOfSight(this.targetObject.transform.position, Mathf.Min(a, (float) this.VisionRange));
  }
  ...
  {
    return GameManager.RoomActive && this.canLayEggs && (double) this.gm.TimeSince(this.lastLaidEggTimestamp) >= (double) this.minTimeBetweenEggs && EnemyHopper.EnemyHoppers.Count < EnemyHopper.maxHoppersPerRoom && EnemyEgg.EnemyEggs.Count < EnemyHopper.maxEggsPerRoom;
  }

  private bool ShouldSprayPoison() => this.canSprayPoison;
  ...
  private void LayEgg()
  {
    ...
    {
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.eggPrefab, this.transform.position + new Vector3(0.0f, -0.5f, 0.0f), Quaternion.identity, (UnityEngine.Object) this.transform.parent != (UnityEngine.Object) null ? this.transform.parent : (Transform) null);
