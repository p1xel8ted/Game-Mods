...
public class HUD_BlackSoul : BaseMonoBehaviour
{
  ...
  private void Start()
  {
    ...
    this.ProgressBar.fillAmount = this.ProgressBarInstant.fillAmount = (float) Inventory.BlackSouls / this.GetUpgradeTarget();
  ...
  private IEnumerator EndTutorialRoutine()
  {
    ...
    yield return (object) new WaitForSecondsRealtime(0.5f);
    this.ProgressBarInstant.fillAmount = (float) Inventory.BlackSouls / this.GetUpgradeTarget();
    Progress = 0.0f;
    Duration = 0.5f;
    float StartFill = this.ProgressBar.fillAmount;
    float EndFill = (float) Inventory.BlackSouls / this.GetUpgradeTarget();
  ...
  private void OnGetBlackSoul(int DeltaValue)
  {
    ...
    if ((double) Inventory.BlackSouls / (double) this.GetUpgradeTarget() >= 1.0)
    {
