...
public class Interaction_Offering : Interaction
{
  ...
  private void GiveResource() => Inventory.AddItem((int) this.Resource, 1);

  private void StartCombat() => this.StartCoroutine((IEnumerator) this.StartCombatRoutine());
  ...
  private IEnumerator DelayGoTo()
  {
    ...
    yield return (object) new WaitForSeconds(0.5f);
    PlayerFarming.Instance.GoToAndStop(interactionOffering.PlayerGoTo, interactionOffering.gameObject, GoToCallback: new System.Action(interactionOffering.CollectResources));
  }

  private void CollectResources() => this.StartCoroutine((IEnumerator) this.InteractRoutine());
