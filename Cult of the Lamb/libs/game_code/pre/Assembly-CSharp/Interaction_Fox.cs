...
public class Interaction_Fox : Interaction
{
  ...
  private void Yes() => this.StartCoroutine((IEnumerator) this.YesRoutine());
  ...
  private IEnumerator GiveKeyRoutine()
  {
    ...
    RumbleManager.Instance.Rumble();
    yield return (object) new WaitForSeconds(1f);
    interactionFox.KeyPiece.transform.DOMove(PlayerFarming.Instance.gameObject.transform.position, 5f).SetEase<TweenerCore<Vector3, Vector3, VectorOptions>>(Ease.InCirc);
