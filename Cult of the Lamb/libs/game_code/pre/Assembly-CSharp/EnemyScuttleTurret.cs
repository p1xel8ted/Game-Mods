...
public class EnemyScuttleTurret : UnitObject
{
  ...
  private IEnumerator ShootArrowRoutine()
  {
    ...
    while (--i >= 0)
    {
      ...
      float Progress = 0.0f;
      while ((double) (Progress += Time.deltaTime) < (double) _timeBetweenShooting)
      {
        if (!secondaryAttack)
  ...
  private IEnumerator MegaAttackIE()
  {
    ...
    {
      enemyScuttleTurret.spine.AnimationState.SetAnimation(0, "attack-shoot", false);
      yield return (object) new WaitForSeconds(enemyScuttleTurret.projectilePattern.Waves[i].FinishDelay);
    }
    enemyScuttleTurret.spine.AnimationState.AddAnimation(0, "animation", true, 0.0f);
