...
public class LongGrass : BaseMonoBehaviour
{
  ...
  private void Start() => this.FrameIntervalOffset = UnityEngine.Random.Range(0, this.UpdateInterval);
  ...
  private void DropHeartChance()
  {
    ...
    if (Interaction_BlueHeart.Hearts.Count + Interaction_RedHeart.Hearts.Count > 1)
      max *= 3;
    if (max == 0 || UnityEngine.Random.Range(0, max) != 17)
      return;
    ...
    Health.AttackFlags AttackFlags)
  {
    this.health.OnDie -= new Health.DieAction(this.OnDie);
    this.Dead = true;
    if ((UnityEngine.Object) PlayerFarming.Instance != (UnityEngine.Object) null && DifficultyManager.AssistModeEnabled && (double) PlayerFarming.Instance.health.HP + (double) PlayerFarming.Instance.health.SpiritHearts + (double) PlayerFarming.Instance.health.BlueHearts == 1.0)
    ...
    if (this.NonCutGrassSprites.Count > 0)
    {
      for (int index = 0; index < this.GrassObjects.Count; ++index)
      {
        if (index < this.GrassObjects.Count - 1 && this.GrassObjects != null && (UnityEngine.Object) this.GrassObjects[index].spriteRederer != (UnityEngine.Object) null && this.NonCutGrassSprites.Count > 0)
        {
          ...
          this.GrassObjects[index].spriteRederer.sprite = this.NonCutGrassSprites[index];
  ...
  {
    if (PerformanceTest.ReduceCPU || (Time.frameCount + this.FrameIntervalOffset) % this.UpdateInterval != 0)
      return;
