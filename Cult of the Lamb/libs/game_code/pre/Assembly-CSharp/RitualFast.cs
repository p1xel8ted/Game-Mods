...
public class RitualFast : Ritual
{
  ...
  private Vector3 SermonEffectStartingPos;

  protected override UpgradeSystem.Type RitualType => UpgradeSystem.Type.Ritual_Fast;
  ...
  private IEnumerator RitualRoutine()
  {
    ...
    {
      if (objective.Type == Objectives.TYPES.PERFORM_RITUAL && ((Objectives_PerformRitual) objective).Ritual == UpgradeSystem.Type.Ritual_Fast)
      {
        completedQuest = true;
        break;
      }
    ...
    CultFaithManager.AddThought(Thought.Cult_Fast, faithMultiplier: completedQuest ? 0.0f : 1f);
  }
