...
public class RitualWorkThroughNight : Ritual
{
  ...
  protected override UpgradeSystem.Type RitualType => UpgradeSystem.Type.Ritual_WorkThroughNight;
  ...
  private IEnumerator RitualRoutine()
  {
    ...
    foreach (FollowerBrain brain in Ritual.FollowerToAttendSermon)
    {
      ...
      FollowerManager.FindFollowerByID(brain.Info.ID).Spine.AnimationState.Event -= new Spine.AnimationState.TrackEntryEventDelegate(workThroughNight.HandleAnimationStateEvent);
    }
    ...
    {
      if (objective.Type == Objectives.TYPES.PERFORM_RITUAL && ((Objectives_PerformRitual) objective).Ritual == UpgradeSystem.Type.Ritual_WorkThroughNight)
      {
        completedQuest = true;
        break;
      }
    ...
    yield return (object) new WaitForSeconds(1f);
    CultFaithManager.AddThought(Thought.Cult_WorkThroughNight, faithMultiplier: completedQuest ? 0.0f : 1f);
  }
  ...
  private IEnumerator MoveFollower(Follower follower, int index)
  {
    ...
    double num = (double) follower.SetBodyAnimation(anims[(int) Mathf.Repeat((float) index, 3f)], true);
