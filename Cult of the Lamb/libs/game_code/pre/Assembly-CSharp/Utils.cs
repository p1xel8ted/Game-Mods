...
public class Utils : BaseMonoBehaviour
{
  ...
  {
    return Mathf.Repeat(Mathf.Atan2(toPosition.y - fromPosition.y, toPosition.x - fromPosition.x), 6.28318548f);
  }
  ...
  {
    int index1 = p.Count - 1;
    bool flag = false;
    for (int index2 = 0; index2 < p.Count; index1 = index2++)
      flag = ((flag ? 1 : 0) ^ (!((double) p[index2].y > (double) v.y ^ (double) p[index1].y > (double) v.y) ? 0 : ((double) v.x < ((double) p[index1].x - (double) p[index2].x) * ((double) v.y - (double) p[index2].y) / ((double) p[index1].y - (double) p[index2].y) + (double) p[index2].x ? 1 : 0))) != 0;
    return flag;
  }
