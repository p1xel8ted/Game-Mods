...
public class RefineryItem : UIInventoryItem, ISelectHandler, IEventSystemHandler, IDeselectHandler
{
    ...
    bool showQuantity = true)
  {
    ...
    this._costText.text = StructuresData.ItemCost.GetCostString(Structures_Refinery.GetCost(type));
  ...
  public override void UpdateQuantity()
  {
    ...
    this._costText.text = StructuresData.ItemCost.GetCostString(Structures_Refinery.GetCost(this.Type));
    if (!this._queued)
    {
      this._button.Confirmable = this._canAfford;
  ...
  private bool CheckCanAfford(InventoryItem.ITEM_TYPE type)
  {
    if (CheatConsole.BuildingsFree)
      return true;
    foreach (StructuresData.ItemCost itemCost in Structures_Refinery.GetCost(type))
    {
