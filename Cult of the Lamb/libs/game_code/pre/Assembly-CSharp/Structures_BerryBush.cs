...
public class Structures_BerryBush : StructureBrain, ITaskProvider
{
  ...
  public void AddBerriesToChest(FollowerLocation Location)
  {
    if (Location == PlayerFarming.Location)
      return;
    List<Structures_CollectedResourceChest> structuresOfType = StructureManager.GetAllStructuresOfType<Structures_CollectedResourceChest>(Location);
    if (structuresOfType.Count <= 0)
      return;
    foreach (InventoryItem.ITEM_TYPE berry in this.GetBerries())
      structuresOfType[0].AddItem(berry, 1);
  }

  public List<InventoryItem.ITEM_TYPE> GetBerries()
  {
    List<InventoryItem.ITEM_TYPE> berries = new List<InventoryItem.ITEM_TYPE>();
    int num = UnityEngine.Random.Range(this.Data.LootCountToDropRange.x, this.Data.LootCountToDropRange.y);
    if (this.IsCrop)
    {
      num = UnityEngine.Random.Range(this.Data.CropLootCountToDropRange.x, this.Data.CropLootCountToDropRange.y);
