...
public class RitualConsumeFollower : Ritual
{
  ...
  protected override UpgradeSystem.Type RitualType => UpgradeSystem.Type.Ritual_ConsumeFollower;
  ...
  private IEnumerator HeartsOfTheFaithfulRitual()
  {
    ...
    followerSelectInstance.Show(Ritual.GetFollowersAvailableToAttendSermon(), followerSelectionType: ritualConsumeFollower.RitualType);
    ...
    yield return (object) new WaitForSeconds(1f);
    int Delta = Mathf.CeilToInt((float) (DataManager.TargetXP[Mathf.Min(DataManager.Instance.Level, DataManager.TargetXP.Count - 1)] - DataManager.Instance.XP));
    ...
    {
      if (objective.Type == Objectives.TYPES.PERFORM_RITUAL && ((Objectives_PerformRitual) objective).Ritual == UpgradeSystem.Type.Ritual_ConsumeFollower && ((Objectives_PerformRitual) objective).TargetFollowerID_1 == followerID)
      {
        completedQuest = true;
        break;
      }
    ...
    yield return (object) new WaitForSeconds(1f);
    CultFaithManager.AddThought(Thought.Cult_ConsumeFollower, followerID, completedQuest ? 0.0f : 1f);
  }
