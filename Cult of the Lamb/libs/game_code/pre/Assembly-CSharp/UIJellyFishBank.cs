...
public class UIJellyFishBank : BaseMonoBehaviour
{
  ...
  private void UpdateLocalInvestment() => this.localInvestment = DataManager.Instance.Investment;
  ...
  private void SetGraph()
  {
    if (DataManager.Instance.CheckInvestmentExist())
    {
      ...
      foreach (GameObject graphPoint in this.GraphPoints)
      {
        ...
        {
          Debug.Log((object) $"Checking index of investment day: {(object) index}Investment Count = {(object) DataManager.Instance.Investment.InvestmentDays.Count}");
  ...
  {
    if (_Withdrawl)
      this.ConfirmationText.text = "Withdrawl: " + (object) this.InvestmentAdd;
    else
      this.ConfirmationText.text = "Deposit: " + (object) this.InvestmentAdd;
  }
    ...
    if (DataManager.Instance.CheckInvestmentExist())
    {
      ...
      if (num > 0)
      {
        for (int index = 0; index < num; ++index)
        {
          ...
          Debug.Log((object) $"Run Investments, day : {(object) fishInvestmentDay.Day} Interest Earned: {(object) fishInvestmentDay.InvestmentAmount}");
        }
      ...
      {
        this.SetGraph();
        Debug.Log((object) $"Don't run investments, last invest: {(object) DataManager.Instance.Investment.LastDayCheckedInvestment}Current Day: {(object) TimeManager.CurrentDay}");
      }
  ...
  {
    Debug.Log((object) ("Calculating Investment of " + (object) InvestmentAmount));
    ...
    Debug.Log((object) $"Investment Ran: {(object) x}Return of: {(object) (float) ((double) InvestmentAmount * (double) y)}");
  ...
  public void SetInvestment(int InvestmentAdd)
  {
    ...
    if (DataManager.Instance.CheckInvestmentExist())
    {
      ...
      {
        Debug.Log((object) $"Create Investment Day, Day: {(object) TimeManager.CurrentDay} Total Investment On Day: {(object) DataManager.Instance.Investment.InitialInvestment}");
