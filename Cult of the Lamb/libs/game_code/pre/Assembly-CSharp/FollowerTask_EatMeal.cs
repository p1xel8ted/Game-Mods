...
public class FollowerTask_EatMeal : FollowerTask
{
  ...
  protected override int GetSubTaskCode() => this._mealID;
  ...
  protected override void TaskTick(float deltaGameTime)
  {
    ...
    if ((double) this._progress < 15.0)
      return;
    this._brain.Stats.Satiation += (float) CookingData.GetSatationAmount(CookingData.GetMealFromStructureType(this.MealType));
    HungerBar.ReservedSatiation -= (float) CookingData.GetSatationAmount(CookingData.GetMealFromStructureType(this.MealType)) / FollowerManager.GetTotalNonLockedFollowers();
    this.End();
  }
  ...
  {
    base.OnAbort();
    if (this.State == FollowerTaskState.Doing && !this._mealEatenByPlayer)
    {
      this._brain.Stats.Satiation += (float) CookingData.GetSatationAmount(CookingData.GetMealFromStructureType(this.MealType));
      HungerBar.ReservedSatiation -= (float) CookingData.GetSatationAmount(CookingData.GetMealFromStructureType(this.MealType)) / FollowerManager.GetTotalNonLockedFollowers();
      this.AddThought();
