...
public class WildLifeDangerous : UnitObject
{
  ...
  private void ChangeState(StateMachine.State newState)
  {
    ...
    switch (newState)
    {
      ...
      case StateMachine.State.RaiseAlarm:
        this.spriteRenderer.color = Color.white;
        if (this.TargetEnemy.gameObject.tag == "Player")
        {
  ...
  private void LookOutForDanger()
  {
    ...
    foreach (Health allUnit in Health.allUnits)
    {
      if (allUnit.team != this.health.team && (double) Vector2.Distance((Vector2) this.transform.position, (Vector2) allUnit.gameObject.transform.position) < 5.0 && allUnit.team != Health.Team.Neutral && !allUnit.untouchable)
      {
        ...
        if ((double) angle < (double) this.state.facingAngle + 45.0 && (double) angle > (double) this.state.facingAngle - 45.0 && this.CheckLineOfSight(allUnit.gameObject.transform.position, 5f))
        {
