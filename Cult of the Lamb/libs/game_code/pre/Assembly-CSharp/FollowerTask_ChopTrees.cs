...
public class FollowerTask_ChopTrees : FollowerTask_AssistPlayerBase
{
  ...
  private bool BigTreeVariant => this._tree != null && this._tree.Data.VariantIndex == 2;
  ...
  private Structures_Tree GetNextTree()
  {
    ...
    List<Structures_Tree> allAvailableTrees = StructureManager.GetAllAvailableTrees(this.Location);
    foreach (Structures_Tree structuresTree in allAvailableTrees)
    {
      ...
      float num3 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, structuresTree.Data.Position);
      if (!this.BigTreeVariant && (double) num3 < (double) num2 && !structuresTree.Data.IsSapling)
      {
        float num4 = num3 + (structuresTree.Data.Prioritised ? 0.0f : 1000f);
    ...
    if (nextTree == null)
    {
      foreach (Structures_Tree structuresTree in allAvailableTrees)
      {
        ...
        float num5 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, structuresTree.Data.Position);
        if (this.BigTreeVariant && (double) num5 < (double) num2 && !structuresTree.Data.IsSapling)
        {
          float num6 = num5 + (structuresTree.Data.Prioritised ? 0.0f : 1000f);
