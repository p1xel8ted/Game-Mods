...
public class Interaction_GambleStatue : Interaction
{
  ...
  private void Start() => this.SetResources();
  ...
  private IEnumerator PayDevotion()
  {
    ...
    PlayerFarming.Instance.simpleSpineAnimator.AddAnimate("devotion/devotion-loop", 0, true, 0.0f);
    ...
    while ((double) ++NumSouls <= (double) SoulsToGive)
    {
      if ((double) NumSouls == (double) SoulsToGive)
      {
        ...
        SoulCustomTarget.Create(interactionGambleStatue.PayResourceReceivePosition, PlayerFarming.Instance.CameraBone.transform.position, Color.white, new System.Action(interactionGambleStatue.\u003CPayDevotion\u003Eb__22_0));
        yield return (object) new WaitForSeconds(0.5f);
      }
      else
      {
        SoulCustomTarget.Create(interactionGambleStatue.PayResourceReceivePosition, PlayerFarming.Instance.CameraBone.transform.position, Color.white, (System.Action) null);
  ...
  private void Gamble() => this.StartCoroutine((IEnumerator) this.GambleRoutine());
