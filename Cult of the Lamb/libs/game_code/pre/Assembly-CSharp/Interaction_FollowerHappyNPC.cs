...
public class Interaction_FollowerHappyNPC : Interaction
{
  ...
  private IEnumerator FirstChatIE()
  {
    ...
    PlayerFarming.Instance.state.facingAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, followerHappyNpc.transform.position);
    PlayerFarming.Instance.state.LookAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, followerHappyNpc.transform.position);
  ...
  {
    Interaction_FollowerHappyNPC followerHappyNpc = this;
    PlayerFarming.Instance.GoToAndStop(followerHappyNpc.transform.position + Vector3.left * 2f);
    ...
    followerSelectInstance.Show(FollowerManager.SimFollowersAtLocation(FollowerLocation.Base), (List<SimFollower>) null, false, UpgradeSystem.Type.Count, true, true, true);
    ...
    selectMenuController2.OnFollowerSelected = selectMenuController2.OnFollowerSelected + (System.Action<FollowerInfo>) (followerInfo =>
    {
  ...
  private IEnumerator GiveFollowerReward(
    ...
    Follower follower)
  {
    ...
    follower.GetComponentInChildren<BarController>(true).SetBarSize(follower.Brain.Stats.Adoration / follower.Brain.Stats.MAX_ADORATION, false, true);
    bool waiting = true;
