...
public class RitualHarvest : Ritual
{
  ...
  protected override UpgradeSystem.Type RitualType => UpgradeSystem.Type.Ritual_HarvestRitual;
    ...
    if (plots.Count > 0)
    {
      ...
      foreach (FarmPlot farmPlot1 in FarmPlot.FarmPlots)
      {
        if (farmPlot1.StructureInfo.ID == structuresFarmerPlot.Data.ID)
        {
          ...
          {
            if ((double) Vector3.Distance(farmPlot2.Position, farmPlot1.Position) < 5.0)
            {
              ritualHarvest.averagePosition += farmPlot2.Position;
              ++num;
            }
          }
