...
public class FollowerTask_ClearWeeds : FollowerTask_AssistPlayerBase
{
  ...
  private Structures_Weeds GetNextWeed()
  {
    ...
    foreach (Structures_Weeds allAvailableWeed in StructureManager.GetAllAvailableWeeds(this.Location))
    {
      if ((Object) followerById == (Object) null)
      {
        nextWeed = allAvailableWeed;
        break;
      }
      if (allAvailableWeed.Data.PrioritisedAsBuildingObstruction)
      {
        float num3 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, allAvailableWeed.Data.Position);
        if ((double) num3 < (double) num2)
        {
          float num4 = num3 + (allAvailableWeed.Data.Prioritised ? 0.0f : 1000f);
