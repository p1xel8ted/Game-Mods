...
public class Interaction_DungeonChallenge : Interaction
{
  ...
  private IEnumerator GiveRewardIE()
  {
    ...
    PlayerFarming.Instance.GoToAndStop(dungeonChallenge.transform.position + Vector3.left * 2f);
    ...
    PlayerFarming.Instance.state.facingAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dungeonChallenge.transform.position);
    PlayerFarming.Instance.state.LookAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dungeonChallenge.transform.position);
  ...
  private IEnumerator GiveChallengeIE()
  {
    ...
    PlayerFarming.Instance.GoToAndStop(dungeonChallenge1.transform.position + Vector3.left * 2f);
    while (PlayerFarming.Instance.GoToAndStopping)
      yield return (object) null;
    Health.team2.Clear();
    ...
    Entries[1].soundPath = "event:/dialogue/ratoo/standard_ratoo";
    Entries[2].soundPath = "event:/dialogue/ratoo/standard_ratoo";
    PlayerFarming.Instance.state.facingAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dungeonChallenge1.transform.position);
    PlayerFarming.Instance.state.LookAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dungeonChallenge1.transform.position);
    MMConversation.Play(new ConversationObject(Entries, (List<MMTools.Response>) null, (System.Action) null));
    MMConversation.mmConversation.SpeechBubble.ScreenOffset = 200f;
    while (MMConversation.CURRENT_CONVERSATION != null)
      yield return (object) null;
