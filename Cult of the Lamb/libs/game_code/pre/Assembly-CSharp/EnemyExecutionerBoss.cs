...
public class EnemyExecutionerBoss : BaseMonoBehaviour
{
  ...
  private void OnEnable() => this.health.OnDie += new Health.DieAction(this.OnDie);
  ...
  private void OnHeartTaken() => DataManager.Instance.DefeatedExecutioner = true;
  ...
  private void Update()
  {
    ...
    {
      if ((UnityEngine.Object) (this.Player = GameObject.FindWithTag("Player")) == (UnityEngine.Object) null || (double) Vector3.Distance(this.transform.position + this.ActivateOffset, this.Player.transform.position) >= (double) this.ActivateDistance)
        return;
      this.Play();
    }
  }
