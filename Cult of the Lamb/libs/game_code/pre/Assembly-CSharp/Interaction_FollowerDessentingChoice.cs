...
public class Interaction_FollowerDessentingChoice : Interaction
{
  ...
  [SerializeField]
  private Interaction_FollowerDessentingChoice.Question[] questions;
  ...
  private void Start()
  {
    ...
    this.eventlistener.SetPitchAndVibrator(this.followerInfo.follower_pitch, this.followerInfo.follower_vibrato);
  }
  ...
  private IEnumerator InteractIE()
  {
    ...
    PlayerFarming.Instance.GoToAndStop(dessentingChoice.transform.position + Vector3.left * 2f);
    while (PlayerFarming.Instance.GoToAndStopping)
      yield return (object) null;
    PlayerFarming.Instance.state.facingAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dessentingChoice.transform.position);
    PlayerFarming.Instance.state.LookAngle = Utils.GetAngle(PlayerFarming.Instance.transform.position, dessentingChoice.transform.position);
  ...
  private IEnumerator ResponseIE(
    bool responseWasA,
  ...
  private Interaction_FollowerDessentingChoice.Question GetRandomQuestion()
  {
  ...
  private IEnumerator RecruitedFollower()
  {
    ...
    dessentingChoice.receiveLoop = AudioManager.Instance.CreateLoop("event:/player/receive_animation_loop", PlayerFarming.Instance.gameObject, true);
