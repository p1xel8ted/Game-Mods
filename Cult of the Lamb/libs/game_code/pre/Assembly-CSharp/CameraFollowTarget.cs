...
public class CameraFollowTarget : BaseMonoBehaviour
{
  ...
  public void ClearAllTargets() => this.targets.Clear();

  private void Awake() => CameraFollowTarget.Instance = this;
  ...
  private void LateUpdate()
  {
    if (!this.init)
    {
      ...
      {
        GameObject gameObjectWithTag = GameObject.FindGameObjectWithTag("Player Camera Bone");
        if ((UnityEngine.Object) gameObjectWithTag != (UnityEngine.Object) null)
          this.AddTarget(gameObjectWithTag, 1f);
        if (this.targets.Count <= 0)
          return;
      }
      this.init = true;
    ...
    else if (this.targets != null && this.targets.Count > 0)
    {
      ...
      {
        if (this.targets[index] == null || (UnityEngine.Object) this.targets[index].gameObject == (UnityEngine.Object) null)
          this.RemoveTarget(this.targets[index].gameObject);
      }
      ...
      {
        this.distance = this.targets.Count <= 1 || this.SnappyMovement ? Mathf.SmoothDamp(this.distance, this.targetDistance, ref this.SmoothZoom, 0.0166666675f * this.ZoomSpeed, float.PositiveInfinity, this.SnappyMovement ? Time.unscaledDeltaTime : Time.deltaTime) : Mathf.SmoothDamp(this.distance, Mathf.Lerp(this.MinZoom, this.MaxZoom, Mathf.Max(this.bounds.size.x, this.bounds.size.y) / this.ZoomLimiter), ref this.SmoothZoom, 0.0166666675f * this.ZoomSpeed, float.PositiveInfinity, this.SnappyMovement ? Time.unscaledDeltaTime : Time.deltaTime);
      ...
      this.CurrentPosition = !MMConversation.isPlaying || this.SnappyMovement ? Vector3.Lerp(this.CurrentPosition, vector3, 5f * Time.unscaledDeltaTime) : Vector3.SmoothDamp(this.CurrentPosition, vector3, ref this.CurrentPositionVelocity, 0.2f, this.CurrentPositionMaxSpeed, Time.unscaledDeltaTime);
      ...
      case 1:
        ...
        if (this.UseCameraLimit)
        {
          ...
          if ((double) centrePoint.y >= (double) this.CameraLimitBounds.center.y + (double) this.CameraLimitBounds.extents.y)
            this.Look = Vector3.zero;
          centrePoint.y = Mathf.Min(this.CameraLimitBounds.center.x + this.CameraLimitBounds.extents.y, centrePoint.y);
          if ((double) centrePoint.y <= (double) this.CameraLimitBounds.center.y - (double) this.CameraLimitBounds.extents.y)
            this.Look = Vector3.zero;
          centrePoint.y = Mathf.Max(this.CameraLimitBounds.center.x - this.CameraLimitBounds.extents.y, centrePoint.y);
        }
        return centrePoint;
      default:
  ...
  private IEnumerator ResetTargetCameraRoutine()
  {
    ...
    while ((double) (Progress += Time.unscaledDeltaTime) < (double) cameraFollowTarget.TargetCameraTransitionDuration)
    {
      ...
      cameraFollowTarget.CurrentTargetCameraPosition = Vector3.Lerp(StartingPosition, cameraFollowTarget.CurrentPosition, cameraFollowTarget.TargetCameraAnimationCurve.Evaluate(Progress / cameraFollowTarget.TargetCameraTransitionDuration));
      cameraFollowTarget.transform.rotation = Quaternion.Slerp(StartingAngle, Quaternion.Euler(new Vector3(-45f, 0.0f, 0.0f)), cameraFollowTarget.TargetCameraAnimationCurve.Evaluate(Progress / cameraFollowTarget.TargetCameraTransitionDuration));
      yield return (object) null;
    }
    cameraFollowTarget.TargetCamera = (Camera) null;
  }

  private void OnPreRender() => GL.wireframe = CheatConsole.WIREFRAME_ENABLED;

  private void OnPostRender() => GL.wireframe = false;
