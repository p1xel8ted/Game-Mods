...
public class FollowerTask_Forage : FollowerTask_AssistPlayerBase
{
  ...
  private Structures_BerryBush GetNextBush()
  {
    ...
    foreach (Structures_BerryBush allAvailableBush in StructureManager.GetAllAvailableBushes(this.Location))
    {
      if ((Object) followerById == (Object) null)
      {
        nextBush = allAvailableBush;
        break;
      }
      float num3 = Vector3.Distance(this._helpingPlayer ? instance.transform.position : followerById.transform.position, allAvailableBush.Data.Position);
      if ((double) num3 < (double) num2)
      {
        float num4 = num3 + (allAvailableBush.Data.Prioritised ? 0.0f : 1000f);
