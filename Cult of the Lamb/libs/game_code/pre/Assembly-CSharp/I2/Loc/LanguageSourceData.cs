...
public class LanguageSourceData
{
  ...
  private bool TryGetFallbackTranslation(
    ...
    bool skipDisabled = false)
  {
    string source = this.mLanguages[langIndex].Code;
    if (!string.IsNullOrEmpty(source))
    {
      ...
      for (int index = 0; index < this.mLanguages.Count; ++index)
      {
        if (index != langIndex && this.mLanguages[index].Code.StartsWith(source, StringComparison.Ordinal) && (!skipDisabled || this.mLanguages[index].IsEnabled()))
        {
    ...
    for (int index = 0; index < this.mLanguages.Count; ++index)
    {
      if (index != langIndex && (!skipDisabled || this.mLanguages[index].IsEnabled()) && (source == null || !this.mLanguages[index].Code.StartsWith(source, StringComparison.Ordinal)))
      {
        Translation = termData.GetTranslation(index, overrideSpecialization, true);
        if (!string.IsNullOrEmpty(Translation))
          return true;
