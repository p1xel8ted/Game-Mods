...
public class IllnessBar : MonoBehaviour
{
  ...
  private void OnDestroy() => TimeManager.OnNewPhaseStarted -= new System.Action(this.DecreaseDynamicMax);
  ...
  public static void UpdateSimulation(float DeltaGameTime)
  {
    ...
    if (!DataManager.Instance.OnboardedSickFollower && PlayerFarming.Location != FollowerLocation.Base || TimeManager.IsNight || DataManager.Instance.Followers.Count == 0 || (double) TimeManager.TotalElapsedGameTime - (double) DataManager.Instance.LastFollowerToBecomeIll < 600.0 / (double) DifficultyManager.GetTimeBetweenIllnessMultiplier() || 1.0 - (double) IllnessBar.Count / (double) IllnessBar.Max >= 0.25)
      return;
  ...
  private IEnumerator UpdateRoutine()
  {
    ...
    while (true)
    {
      while (!DataManager.Instance.ShowCultIllness)
        yield return (object) null;
      illnessBar.BarController.SetBarSize((float) (1.0 - (double) IllnessBar.Count / (double) IllnessBar.Max), true);
      if (1.0 - (double) IllnessBar.Count / (double) IllnessBar.Max < 0.25)
      {
        if (illnessBar.sequence == null)
        {
          ...
          illnessBar.PulseSequence.AppendCallback(new TweenCallback(illnessBar.\u003CUpdateRoutine\u003Eb__29_0));
      ...
      else if (illnessBar.sequence != null)
      {
