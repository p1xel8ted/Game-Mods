  ...
  private void Hatch()
  {
    ...
    for (int index = 0; index < this.numHatchlings; ++index)
    {
      ...
      if (!(bool) Physics2D.Raycast((Vector2) this.transform.position, (Vector2) vector3.normalized, this.hatchlingDistanceFromEgg, (int) this.layerToCheck))
        Addressables.InstantiateAsync((object) this.EnemyPrefab, this.transform.position + vector3, Quaternion.identity, this.transform.parent).Completed += (System.Action<AsyncOperationHandle<GameObject>>) (obj =>
        {
          GameObject gameObject = obj.Result.gameObject;
          ...
          UnitObject component3 = gameObject.GetComponent<UnitObject>();
