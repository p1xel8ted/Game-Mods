...
public class Task_Combat : Task
{
  ...
  public void GetNewTarget()
  {
    ...
    {
      if (allUnit.team != this.health.team && !allUnit.InanimateObject && allUnit.team != Health.Team.Neutral && (double) Vector2.Distance((Vector2) this.t.transform.position, (Vector2) allUnit.gameObject.transform.position) < (double) this.DetectEnemyRange && this.t.CheckLineOfSight(allUnit.gameObject.transform.position, Vector2.Distance((Vector2) allUnit.gameObject.transform.position, (Vector2) this.t.transform.position)))
      {
  ...
  private static int SortByY(Vector2 p1, Vector2 p2) => p1.y.CompareTo(p2.y);
}
