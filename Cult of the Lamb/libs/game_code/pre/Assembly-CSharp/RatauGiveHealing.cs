...
public class RatauGiveHealing : BaseMonoBehaviour
{
  ...
  private IEnumerator WaitForHealing()
  {
    ...
    int num = (int) ratauGiveHealing.receiveLoop.stop(STOP_MODE.ALLOWFADEOUT);
    ...
    Debug.Log((object) $"{(object) (float) ((double) healthPlayer.HP + (double) healthPlayer.SpiritHearts)} - {(object) (float) ((double) healthPlayer.totalHP + (double) healthPlayer.TotalSpiritHearts)}");
    float totalSpiritHearts = healthPlayer.TotalSpiritHearts;
    if (!0.0f.Equals(totalSpiritHearts))
    {
      if (1f.Equals(totalSpiritHearts))
      {
