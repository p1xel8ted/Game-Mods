...
executable_name="Cult of the Lamb"
...
case $os_type in
    ...
    Darwin*)
        ...
        real_executable_name=$(defaults read "$BASEDIR/Contents/Info" CFBundleExecutable)
        executable_path="$BASEDIR/Contents/MacOS/${real_executable_name}"
        lib_postfix="dylib"
        ;;
    *)
...
if [ -n "$1" ]; then
    case $os_type in
        ...
        Darwin*)
            ...
            full_path_part=$(echo "$1" | grep "/Contents/MacOS")
